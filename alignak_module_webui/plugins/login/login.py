...
def user_login():
    logger.debug("user login request, remote user enabled: %s: %s",
                 app.remote_user_enable, app.remote_user_variable)
    for header in app.request.headers:
        logger.debug("X request header: %s = %s", header, app.request.headers[header])

    err = app.request.GET.get('error', None)
    if err:
        logger.warning("login page with error message: %s", err)

    cookie_value = app.request.get_cookie(app.session_cookie, secret=app.auth_secret)
    if cookie_value:
        logger.info("user login request, existing cookie found: %s", cookie_value)
        bottle.redirect(app.get_url("Dashboard"))

    elif app.remote_user_enable in ['1', '2']:
        logger.debug("user login request, no existing cookie found")
        if not err:
            user_name = None
            if app.remote_user_enable == '1':
                logger.debug("search %s in request headers", app.remote_user_variable)
                if app.remote_user_variable in app.request.headers:
                    user_name = app.request.headers[app.remote_user_variable]
                    logger.debug("remote user found in request headers: %s", user_name)

            if app.remote_user_enable == '2':
                logger.debug("search %s in WSGI environment", app.remote_user_variable)
                if app.remote_user_variable in app.request.environ:
                    user_name = app.request.environ[app.remote_user_variable]
                    logger.debug("remote user found in WSGI environment: %s", user_name)

            if not user_name:
                logger.warning("remote user is enabled but no authenticated "
                               "user name was found")
    ...
    logger.debug("session user message - get: %s",
...
def user_logout():
    ...
    logger.info("user '%s' signed out", contact_name)
    bottle.redirect(app.get_url("GetLogin"))
    return {}
...
def user_auth():
    ...
    logger.info(" user '%s' is signing in ...", login)

    # Tries to authenticate user
    is_authenticated = app.check_authentication(login, password)
    if is_authenticated:
        logger.debug(" setting cookie '%s' for %s", app.session_cookie, login)
        cookie_value = {
            'login': login,
            'session': app.user_session,
            'info': app.user_info
        ...
        try:
            ...
            logger.error(" cookie value error (too long > 4096 bytes!): %s", cookie_value)
            is_authenticated = False
        else:
            logger.debug(" user '%s' signed in: %s", login, cookie_value)
            bottle.redirect(app.get_url("Dashboard"))
    else:
        logger.debug(" user '%s' access denied, redirection to: %s", login, app.get_url("GetLogin"))

        logger.info("refused login message: %s", app.request.environ.get('MSG', ''))
