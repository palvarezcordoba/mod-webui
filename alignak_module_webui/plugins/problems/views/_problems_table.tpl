<table class="table table-condensed" style="table-layout:fixed; width:100%;">
  <tbody>
    ...
    %else:
      <td width="110px" title="{{pb.get_name()}} - {{pb.state}}<br> Since {{helper.print_date(pb.last_state_change, dt_format="%d %b %Y %H:%M:%S")}}<br> Last check {{helper.print_duration(pb.last_chk)}}<br> Next check {{helper.print_duration(pb.next_chk)}}"
