...
print(("Host notifications since today are: %d"
      % db.logs.find({"$and": [{"type": "HOST NOTIFICATION"},
                               {"time": {"$gte": today_beginning_time}}]}).count()))

print(("Host notifications yesterday are: %d"
      % db.logs.find({"$and": [{"type": "HOST NOTIFICATION"},
                               {"time": {"$gte": yesterday_beginning_time}},
                               {"time": {"$lte": yesterday_end_time}}]}).count()))

print(("Host and service notifications logs are: %d"
      % db.logs.find({"type": {"$in": ["HOST NOTIFICATION", "SERVICE NOTIFICATION"]}}).count()))

print(("Configured logs type logs are: %d"
      % db.logs.find({"type": {"$in": logs_type}}).count()))

print(("Configured hosts logs are: %d"
      % db.logs.find({"host_name": {"$in": logs_hosts}}).count()))

print(("Configured services logs are: %d"
      % db.logs.find({"service_description": {"$in": logs_services}}).count()))
